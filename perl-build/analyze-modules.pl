...
my $doc = $xml->XMLin($XML_FILE); # $doc is a reference to a hash of hashes of hashes
...
#       print "3: \t\t$k3 => $rHoHoH->{ $k1 }->{ $k2 }->{ $k3 }\n";

#       for my $k4 ( sort keys %{$rHoHoH->{ $k1 }->{ $k2 }->{ $k3 }} ) {
# 	print "4: \t\t\t$k4 => $rHoHoH->{ $k1 }->{ $k2 }->{ $k3 }->{ $k4 }\n";
#       }
#     }
#   }
# }
